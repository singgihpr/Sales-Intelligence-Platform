          ...
          <div className="space-y-6">
            {showTargetForm && (
              <form onSubmit={(e) => {
                ...
                <div className="md:col-span-1"><label className="block text-xs font-medium text-slate-500 mb-1">{t('adminDashboard.targets.headers.targetBE')}</label><input type="number" step="0.1" required value={targetForm.target_be} onChange={e=>setTargetForm({...targetForm, target_be:e.target.value===''?'':Number(e.target.value)})} className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-transparent" /></div>
