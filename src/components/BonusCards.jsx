...
export const BonusSummaryCard = ({ bonusSummary, hint }) => {
  const { t } = useTranslation();
  const { percentage, volume, activeOutlets, total } = bonusSummary || {};
  return (
    <div className="relative bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-5 text-white shadow-sm border border-slate-800">
      {hint && <Hint text={hint} />}
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">{t('bonusCards.totalEstBonus')}</p>
      ...
      <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-slate-700/50">
        <div className="text-center">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('bonusCards.percentage')}</p>
          <p className="text-sm font-bold text-emerald-300">{formatRp(percentage?.bonus || 0)}</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('bonusCards.volume')}</p>
          <p className="text-sm font-bold text-blue-300">{formatRp(volume?.bonus || 0)}</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('bonusCards.activeOutlets')}</p>
...
export const PercentageBonusCard = ({ currentBE, targetBE, config, result, hint }) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-4">
      {hint && <Hint text={hint} />}
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2"><Target className="w-4 h-4 text-emerald-600" /> {t('bonusCards.percentageBased')}</h3>
        <span className="text-xs font-bold text-slate-400">{t('bonusCards.base', { value: formatRp(config.base_reward) })}</span>
      </div>
      <BonusProgressBar label={t('bonusCards.attainment')} current={currentBE} max={targetBE} markers={config.tiers.map(t => (t.threshold / 100) * targetBE)} />
      ...
      <div className="pt-2 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
        <span className="text-xs text-slate-500">{t('bonusCards.current')}</span>
      ...
      <div className="pt-2 border-t border-slate-50 dark:border-slate-800">
        ...
        >
          ...
          {isSimulating ? t('bonusCards.closeSim') : t('bonusCards.openSim')}
        ...
        {isSimulating && (
          <div className="mt-3 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-500 uppercase">{t('bonusCards.projectedBE')}</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center">
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">{t('bonusCards.attainment')}</p>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center">
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">{t('bonusCards.estBonus')}</p>
            ...
            {simTier && currentTier && simTier.reward !== currentTier.reward && (
              <div className={`text-xs font-bold text-center py-2.5 rounded-lg ${simTier.reward > currentTier.reward ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-red-50 text-red-700 dark:bg-red-950/30 dark:text-red-400'}`}>
                {t('bonusCards.vsCurrent', { value: `${simTier.reward > currentTier.reward ? '+' : ''}${formatRp(simTier.reward - currentTier.reward)}` })}
              </div>
            )}
          </div>
        )}
...
export const VolumeBonusCard = ({ currentBE, config, result, hint }) => {
  const { t } = useTranslation();
  const [isSimulating, setIsSimulating] = useState(false);
  const [simulatedBE, setSimulatedBE] = useState(currentBE);
  if (!config || !config.tiers) return null;
  ...
  return (
    <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-4">
      {hint && <Hint text={hint} />}
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2"><TrendingUp className="w-4 h-4 text-blue-600" /> {t('bonusCards.volumeBased')}</h3>
      </div>
      <BonusProgressBar label={t('bonusCards.volume')} current={currentBE} max={maxThreshold} markers={config.tiers.map(t => t.threshold)} colorClass="bg-blue-500" />
      ...
      <div className="pt-2 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
        <span className="text-xs text-slate-500">{t('bonusCards.currentReward')}</span>
...
export const ActiveOutletsBonusCard = ({ totalAssigned, activeCount, config, result, hint }) => {
  const { t } = useTranslation();
  const [isSimulating, setIsSimulating] = useState(false);
  ...
  return (
    <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-5 space-y-4">
      {hint && <Hint text={hint} />}
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2"><Store className="w-4 h-4 text-amber-600" /> {t('bonusCards.activeOutletsBased')}</h3>
        <span className="text-xs font-bold text-slate-400">{t('bonusCards.base', { value: formatRp(config.base_reward) })}</span>
      </div>
      <BonusProgressBar label={t('bonusCards.activeRate')} current={activeCount} max={totalAssigned} markers={config.tiers.map(t => (t.threshold / 100) * totalAssigned)} colorClass="bg-amber-500" />
      <div className="flex justify-center gap-1 text-xs font-medium text-slate-500">
        <span className="font-bold text-slate-900 dark:text-white">{activeCount}</span> / <span>{totalAssigned}</span> {t('bonusCards.outlets')} active ({Math.round(percent)}%)
      </div>
      <div className="grid grid-cols-3 gap-2 mt-2">
        {config.tiers.map((tier, idx) => (
      ...
      <div className="pt-2 border-t border-slate-50 dark:border-slate-800">
        ...
        {isSimulating && (
          <div className="mt-3 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-500 uppercase">{t('bonusCards.projectedActive')}</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min="0"
              ...
              />
              <div className="flex justify-between text-[10px] text-slate-400 font-medium">
                <span>0</span><span>{totalAssigned} {t('bonusCards.outlets')}</span>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 text-center">
                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-1">{t('bonusCards.activeRate')}</p>
