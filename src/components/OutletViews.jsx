    ...
    <div className="fixed inset-0 z-[70] bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-sm p-6 space-y-4 animate-in slide-in-from-bottom-4 duration-300">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t('outletViews.ohsModal.title')}</h3>
          <button onClick={onClose} className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
            <X className="w-5 h-5 text-slate-500" />
          </button>
        </div>
        <p className="text-xs text-slate-500 leading-relaxed">
          {t('outletViews.ohsModal.desc')}
        </p>
        <div className="space-y-2">
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold text-emerald-800 dark:text-emerald-400">{t('outletViews.ohsModal.healthyLabel')}</p>
              <p className="text-[10px] text-emerald-700/70 dark:text-emerald-500/70">{t('outletViews.ohsModal.healthyDesc')}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold text-amber-800 dark:text-amber-400">{t('outletViews.ohsModal.warningLabel')}</p>
              <p className="text-[10px] text-amber-700/70 dark:text-amber-500/70">{t('outletViews.ohsModal.warningDesc')}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold text-red-800 dark:text-red-400">{t('outletViews.ohsModal.unhealthyLabel')}</p>
              <p className="text-[10px] text-red-700/70 dark:text-red-500/70">{t('outletViews.ohsModal.unhealthyDesc')}</p>
            </div>
          </div>
        </div>
        <div className="text-[10px] text-slate-400 text-center pt-1 space-y-1">
          <p><strong>{t('outletViews.ohsModal.formula')}</strong> {t('outletViews.ohsModal.formulaDesc')}</p>
          <p>{t('outletViews.ohsModal.formulaBreakdown')}</p>
        </div>
        <button onClick={onClose} className="w-full py-2.5 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 active:scale-95 transition-all">
          {t('outletViews.ohsModal.understood')}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4 pb-32 animate-in fade-in">
      ...
      <div className="flex items-center justify-between px-1">
        <p className="text-xs text-slate-500 font-medium">{t('outletViews.found', { count: filtered.length })}</p>
      ...
      <div className="relative">
        ...
        <input 
          type="text" 
          placeholder={t('outletViews.searchPlaceholder')} 
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          >
            <option value="all">{t('outletViews.filterAll')}</option>
            <option value="healthy">{t('outletViews.filterHealthy')}</option>
            <option value="warning">{t('outletViews.filterWarning')}</option>
            <option value="unhealthy">{t('outletViews.filterUnhealthy')}</option>
          </select>
        ...
        <div className="relative flex-1">
          <select 
            ...
            className="w-full appearance-none bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl py-2.5 pl-3 pr-8 text-xs font-bold text-slate-600 focus:ring-2 focus:ring-emerald-500 outline-none"
          >
            <option value="ohs-desc">{t('outletViews.sortOhsDesc')}</option>
            <option value="ohs-asc">{t('outletViews.sortOhsAsc')}</option>
            <option value="be-desc">{t('outletViews.sortBeDesc')}</option>
            <option value="name-asc">{t('outletViews.sortNameAsc')}</option>
          </select>
          <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {paginated.length === 0 && <p className="text-center text-xs text-slate-400 py-8">{t('outletViews.notFound')}</p>}
      </div>
  ...
  return (
    <div className="space-y-6 pb-24 animate-in slide-in-from-right-4 duration-300">
      <button onClick={onBack} className="flex items-center gap-2 text-slate-500 font-bold text-xs uppercase tracking-wider">
        <ArrowLeft className="w-4 h-4" /> {t('outletViews.back')}
      </button>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-slate-500 flex items-center gap-1 mt-1">
              <MapPin className="w-3 h-3" /> {outlet.address || t('outletViews.addressNotAvailable')}
            </p>
            {outlet.branchArea && <p className="text-xs text-slate-400 mt-0.5 font-medium">{t('outletViews.area', { area: outlet.branchArea })}</p>}
          </div>
          <div className="text-right">
            <HealthBadge score={outlet.health} onClick={() => setShowInfo(true)} />
            <p className="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-tight">{t('outletViews.statusActive')}</p>
          </div>
        </div>
      </section>
      ...
      <Card className="bg-gradient-to-br from-emerald-50 to-slate-50 dark:from-emerald-950/20 dark:to-slate-900 border-emerald-100 dark:border-emerald-900/30">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[10px] text-slate-500 uppercase font-bold">{t('outletViews.ohsScore')}</p>
        ...
        <div className="space-y-3">
          <ProgressBar 
            label={t('outletViews.volumeBE40')} 
            value={Math.round(breakdown.volume || 0)} 
            color="bg-emerald-500" 
            percentage={40}
          />
          <ProgressBar 
            label={t('outletViews.trendMoM40')} 
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="p-4">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.totalBE3Mo')}</p>
          <p className="text-lg font-black text-slate-900 dark:text-white mt-1">{(outlet.totalBE3Mo || 0).toFixed(1)}</p>
        </Card>
        <Card className="p-4">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.avgBEMonth')}</p>
          <p className="text-lg font-black text-slate-900 dark:text-white mt-1">{(outlet.avgBE || 0).toFixed(1)}</p>
        </Card>
        <Card className="p-4">
          <div className="flex items-center gap-1.5 mb-1">
            <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.trendMoM')}</p>
        ...
        <Card className="p-4">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.frequency3Mo')}</p>
          <p className="text-lg font-black text-slate-900 dark:text-white mt-1">{outlet.freq3Mo || 0} <span className="text-xs font-medium text-slate-400">{t('outletViews.transactions')}</span></p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-slate-50 dark:bg-slate-800/50 border-none p-4">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.contactPerson')}</p>
          <p className="text-sm font-bold mt-1">{outlet.contact || "Bpk. Manager"}</p>
        </Card>
        <Card className="bg-slate-50 dark:bg-slate-800/50 border-none p-4">
          <p className="text-[10px] text-slate-400 uppercase font-bold">{t('outletViews.lastOrder')}</p>
      ...
      <section>
        <h3 className="font-bold mb-3 flex items-center gap-2 text-sm">
          <History className="w-4 h-4 text-emerald-600" /> {t('outletViews.history3Mo')}
        </h3>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold flex items-center gap-2 text-sm">
            <Calendar className="w-4 h-4 text-emerald-600" /> {t('outletViews.transactionHistory')}
          </h3>
        ...
        <Card className="p-0 overflow-x-auto">
          {historyLoading && transactionHistory.length === 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-slate-400">
              ...
              <p className="text-xs">{t('outletViews.loadingHistory')}</p>
            </div>
          )}
          {historyError && (
            <div className="p-4 text-center">
              <p className="text-xs text-red-500 mb-2">{historyError}</p>
              <button onClick={() => setHistoryPage(1)} className="text-xs text-emerald-600 font-bold">{t('common.tryAgain')}</button>
            </div>
          )}
        ...
        {historyTotal > HISTORY_LIMIT && (
          <div className="flex items-center justify-between mt-3">
            ...
            <span className="text-xs font-bold text-slate-500">
              {t('common.pageOf', { page: historyPage, total: Math.ceil(historyTotal / HISTORY_LIMIT) })}
            </span>
            <button
